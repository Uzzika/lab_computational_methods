...
class GradientFrame(tk.Frame):
    ...
    def _draw_gradient(self, event=None):
        """Рисует градиентный фон внутри фрейма."""
...
class ExperimentApp:
    def __init__(self, root):
        ...
        # Создаем поля ввода и элементы интерфейса
    ...
    def create_input_fields(self):
        ...
        label = tk.Label(self.main_frame, text="Настройки эксперимента", font=("calibri", 20), bg='#12044E', fg='white',
        ...
        # Поле для n
        self.n_label = tk.Label(self.main_frame, text="Количество партий (n):", font=("calibri", 14), anchor='w')
        ...
        # Поле для v
        self.v_label = tk.Label(self.main_frame, text="Влияние дозировки (v):", font=("calibri", 14), anchor='w')
        self.v_label.grid(row=2, column=0, sticky="w", padx=10, pady=5)
        self.v_entry = tk.Entry(self.main_frame, validate='key', bg='white', fg='black', font=("calibri", 12))
        self.v_entry.grid(row=2, column=1, padx=10, sticky="w", pady=5)
        self.v_entry.insert(0, str(self.v_range[0]))

        # Поле для a
        self.a_label = tk.Label(self.main_frame, text="Начальная сахаристость (a):", font=("calibri", 14), anchor='w')
        self.a_label.grid(row=3, column=0, sticky="w", padx=10, pady=5)
        self.a_entry = tk.Entry(self.main_frame, validate='key', bg='white', fg='black', font=("calibri", 12))
        self.a_entry.grid(row=3, column=1, padx=10, sticky="w", pady=5)
        self.a_entry.insert(0, str(self.a_range[0]))

        # Поле для b
        self.b_label = tk.Label(self.main_frame, text="Коэффициент деградации (b):", font=("calibri", 14), anchor='w')
        self.b_label.grid(row=4, column=0, sticky="w", padx=10, pady=5)
        self.b_entry = tk.Entry(self.main_frame, validate='key', bg='white', fg='black', font=("calibri", 12))
        self.b_entry.grid(row=4, column=1, padx=10, sticky="w", pady=5)
